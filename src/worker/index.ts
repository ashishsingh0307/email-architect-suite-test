...
// Get current user
app.get("/api/users/me", devAuth, async (c) => {
  return c.json(c.get("user"));
});

// Get user credits
app.get("/api/users/me/credits", devAuth, async (c) => {
  const db = c.env.DB;
...
app.get("/api/sequences", devAuth, async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
...
// Create sequence
app.post("/api/sequences", devAuth, zValidator("json", CreateSequenceSchema), async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
...
// Update sequence
app.put("/api/sequences/:id", devAuth, async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
...
app.get("/api/sequences/:id", devAuth, async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  const sequenceId = c.req.param("id");
...
// Delete sequence
app.delete("/api/sequences/:id", devAuth, async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  const sequenceId = c.req.param("id");
...
// Duplicate sequence
app.post("/api/sequences/:id/duplicate", devAuth, async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  const sequenceId = c.req.param("id");
...
// Create email block
app.post("/api/sequences/:id/blocks", devAuth, zValidator("json", CreateEmailBlockSchema), async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  const sequenceId = c.req.param("id");
  const body = c.req.valid("json");
...
// Update email block
app.put("/api/blocks/:id", devAuth, zValidator("json", UpdateEmailBlockSchema), async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
...
// Duplicate block
app.post("/api/blocks/:id/duplicate", devAuth, async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  const blockId = c.req.param("id");
  const newBlockId = uuidv4();
...
// Delete block
app.delete("/api/blocks/:id", devAuth, async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  const blockId = c.req.param("id");
...
// Create connection
app.post("/api/connections", devAuth, zValidator("json", CreateConnectionSchema), async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  const body = c.req.valid("json");
...
// Delete connection
app.delete("/api/connections/:id", devAuth, async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
...
app.get("/api/user-templates", devAuth, async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
...
app.delete("/api/user-templates/:id", devAuth, async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
...
app.post("/api/sequences/:id/save-template", devAuth, zValidator("json", CreateTemplateSchema), async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  const sequenceId = c.req.param("id");
  const body = c.req.valid("json");
...
app.post("/api/ai/generate-content", devAuth, zValidator("json", GenerateContentSchema), async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  const body = c.req.valid("json");
...
app.post("/api/auth/dev-login", async (c) => {
  // This creates a dev session token (handled automatically by devAuth)
  return c.json({ success: true });
});

// Rewrite email content with different tone
app.post("/api/ai/rewrite-content", devAuth, async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  const body = await c.req.json();
...
// Export sequence
app.post("/api/sequences/:id/export", devAuth, zValidator("json", ExportSequenceSchema), async (c) => {
  const db = c.env.DB;
  const user = c.get("user");
  const sequenceId = c.req.param("id");
  const body = c.req.valid("json");
